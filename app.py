...
def ai_summary(score, top_factors):
    ...
    first = (
        top_factors[0][0]
        if top_factors
        else "l'absence de signal dominant"
    )

    return (
        f"Le marché pétrole présente un biais {direction}. "
        f"Le facteur dominant détecté est {first}. "
        f"À confirmer avec les prochaines données macro, "
        f"stocks et événements géopolitiques."
    )
...
st.caption(
    "Cockpit pétrole en une page — données indicatives, "
    "actualisation logique toutes les 30 minutes"
)
...
with st.sidebar:
    ...
    st.write(
        "La V1 utilise Yahoo Finance direct pour les prix "
        "et des flux RSS pour les news."
    )
...
with left:
    ...
    for col in ["Prix", "1 jour", "5 jours", "20 jours"]:
        show[col] = show[col].apply(
            lambda x:
            f"{x:,.2f}" if pd.notna(x) and col == "Prix"
